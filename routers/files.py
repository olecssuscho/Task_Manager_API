...
async def download_file(filename:str, task_id:int, user:UserMODELS = Depends(get_current_user),db:AsyncSession = Depends(get_db)):
    ...
    return StreamingResponse(response, media_type=file.content_type, headers={"Content-Disposition": f'attachment; filename="{file.filename}"'},background=BackgroundTask(response.close))