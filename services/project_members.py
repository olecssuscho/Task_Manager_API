...
async def delete_member_services(project_id:int,user_email:str,user:UserDB,db:AsyncSession):
    ...
    if result is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project did not found")
    user = await db.execute(select(UserDB).filter(UserDB.email == user_email))
    user_id = user.scalar_one_or_none()
    if user_id is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User did not found")
    user_db = await db.execute(select(ProjectMemberDB).filter(ProjectMemberDB.project_id == project_id,ProjectMemberDB.user_id == user_id.id))
    if user_db.scalar_one_or_none() is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User did not found")
    await db.execute(delete(ProjectMemberDB).filter(ProjectMemberDB.user_id == user_id.id))
    await db.commit()
    return "Success"
...
async def patch_member_services(project_id:int,user_email:str,role:ProjectMemberDB,user:UserDB,db:AsyncSession):
    ...
    if user_id is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User did not found")
    user_db = await db.execute(select(ProjectMemberDB).filter(ProjectMemberDB.user_id == user_id.id))
    if user_db.scalar_one_or_none() is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User did not found")
    await db.execute(update(ProjectMemberDB).filter(ProjectMemberDB.project_id == project_id,ProjectMemberDB.user_id == user_id.id).values(role = role.role))
    await db.commit()
    return "Success" 
